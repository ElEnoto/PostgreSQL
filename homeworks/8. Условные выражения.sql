...
FROM customers
ORDER BY CASE
             WHEN city IS NULL THEN (contact_name, country)
             ELSE (contact_name, city) END;
...
SELECT concat(first_name, ' ', last_name), COALESCE(NULLIF(title, 'Sales Representative'), 'Sales Stuff')
FROM employees;